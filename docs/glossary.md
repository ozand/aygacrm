...
- **Golden record** — The canonical, merged view of a contact stored in Monica. It is the best current representation of a person after deduplication, enrichment, and conflict resolution.
- **Contact** — The primary CRM entity that Monica manages. A contact represents the canonical relationship record, not just a raw source entry.
- **Person** — A real-world human being. A person may appear in Monica through multiple sources, but Monica stores one contact as the canonical record for that person when possible.
...
- **Source** — An external system or record that contributes data to Monica, such as email, Telegram, LinkedIn, or phone logs.
...
- **Agent** — A non-human actor that uses Monica through API, CLI, or MCP to read data, enrich contacts, or perform controlled writes.
- **Integration** — A connector that syncs data between Monica and an external system or ecosystem app.
